...
MeRGBLineFollower::MeRGBLineFollower(uint8_t port, uint8_t address) : MePort(port)
{
  ...
  else if(address == DEVICE2)
  {
    digitalWrite(s1,LOW);
    digitalWrite(s2,HIGH);
  }
  else if(address == DEVICE3)
  {
    digitalWrite(s1,HIGH);
    digitalWrite(s2,LOW);
  }
  else if(address == DEVICE4)
  {
    digitalWrite(s1,LOW);
    digitalWrite(s2,LOW);
  }
 ...
 * \par Others
 */
bool MeRGBLineFollower::getStudyFlag(void)
{
  return study_flag;
}
...
void MeRGBLineFollower::updataAllSensorValue(void)
{
  ...
  if(return_value == I2C_OK)
  {
    if(i2cData[RGBLINEFOLLOWER_DEVICE_ID_ADDR] == RGBLINEFOLLOWER_DEVICE_ID)
    {
      ...
      study_flag = (i2cData[RGBLINEFOLLOWER_STATE_ADDR]>>4) & 0x01;
    }
    else
    {
      iic_error_count++;  
...
int8_t MeRGBLineFollower::studyBackground(void)
{
  ...
  return_value = writeData(RGBLINEFOLLOWER_STUDY_ADDR, &data, 1);
  return(return_value);
}
